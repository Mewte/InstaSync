...
var queries = function(){
	...
	this.login = function(username, prehash){
		...
		return db.select(["id as user_id","username","hashpw","avatar","bio","created"]).from('users').where({username:username}).limit(1).bind({})
            .then(function(results){
                ...
                this.user = results[0];
                return bcrypt.compare(prehash, this.user.hashpw)
            }).then(function(res){
            	if (res === false){
                    var error = new Error("Invalid username or password.");
                    error.type ="invalid_credentials";
	...
	this.changePassword = function(user_id,currentPass,newPass){
		//refactor this to force the client to send the user it thinks it's logged in as
		//Could cause confusion if a user logs into a different user in a seperate tab
		return db.select(["id as user_id","username","hashpw","avatar","bio","created"]).from('users').where({id:user_id}).limit(1).bind({})
			.then(function(results) {
                if (results.length == 0) {
                    ...
                    this.user = results[0];
                    return bcrypt.compare(currentPass, this.user.hashpw)
                }
            }).then(function(res){
            	if (res === false){
                    var error = new Error("Invalid current password. Please try again.");
				...
				return db('users').update({hashpw: hash}).where({id: this.user.user_id}).limit(1);
			}).then(function(){
				delete this.user.hashpw;
				return this.user;
			}).catch(function(err){throw err;});
